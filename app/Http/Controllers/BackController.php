...
class BackController extends Controller
{
    ...
    public function staff_add_post(Request $request){
        ...
        $request->username == ''  || $request->password == '') {
            return redirect('admin/staff/add')->with(['flash_level' => 'danger', 'flash_message' => 'Vui lòng
            điền vào chỗ trống']);
        }
        $User = new User;
        $User->status = 1;
    ...
    public function system_post(Request $request){
        if ($request->name == '' || $request->email == '' || $request->phone == '') {
            return redirect('admin/system')->with(['flash_level' => 'danger', 'flash_message' => 'Vui lòng
            điền vào chỗ trống']);
        }
    ...
    public function patient_edit_post(Request $request, $id){
        if ($request->fullname == ''||$request->quequan == '') {
            return redirect('admin/patient/edit/' .$id)->with(['flash_level' => 'danger', 'flash_message' => 'Vui lòng
            điền vào chỗ trống']);
        }
        $Patient = Patient::find($id);
    ...
    public function patient_add_post(Request $request){
        if ($request->fullname == '' || $request->quequan == '') {
            return redirect('admin/patient/add')->with(['flash_level' => 'danger', 'flash_message' => 'Vui lòng
            điền vào chỗ trống']);
        }
        $Patient = new Patient;
        $Patient->Status = 1;
